...
int main()
{
	...
	cout << "Задание множеств перечеслительным способом - 1" << endl;		
	cout << "Задание множеств через высказывание - 2" << endl;				
	cin >> menu;															//выбор пользователем способа ввода (число от 1 до 2)

	switch (menu)
	{
	case 1:
		cout << "Введите данные множества A " << endl;						
		for (int j = 1; j <= pwA; j++)
		{
		...
		for (int j = 1; j <= pwB; j++)
		{
			cin >> B[j];													//заполнение множества B вручную
		}
		break;
	...
	while (end != 1)
	{																		//вывод на экран меню с выбором операции
		cout << endl << endl;
		...
		cout << "Выход - 10" << endl;										// 10 - завершение работы программы
		cin >> menu;														//ввод пользоввателем номера операции

		switch (menu)
		{
		case 1:
			Merge(pwA, pwB, pwD, A, B, D);									//вызов функции объединения
			break;															//окончание операции и возврат в меню

		case 2:
			Intersection(pwA, pwB, pwC, A, B, C);							//вызов функции пересечения
			break;															//окончание операции и возврат в меню

		case 3:
			cout << endl << endl;
			cout << "Разность множеств A и B" << endl;						
			Diffrence(pwA, pwB, pwE, A, B, E);								//вызов функции разности
			break;															//окончание операции и возврат в меню

		case 4:
			cout << endl << endl;
			cout << "Разность множеств B и A" << endl;						
			Diffrence(pwB, pwA, pwE, B, A, E);								//вызов функции разности
			break;															//окончание операции и возврат в меню

		case 5:
			SymDiff(pwA, pwB, pwE, pwF, pwG, A, B, E, F, G);				//вызов функции симметрической разности
			break;															//окончание операции и возврат в меню

		case 6:
			Addition(pwA, pwH, pwU, A, H, U);								//вызов функции дополнения
			break;															//окончание операции и возврат в меню

		case 7:
			Addition(pwB, pwK, pwU, B, K, U);                               //вызов функции дополнения
			break;															//окончание операции и возврат в меню

		case 8:
			DecartProizv(pwA, pwB, pwL, A, B, L, n);                        //вызов функции декарового произведения
			break;															//окончание операции и возврат в меню

		case 9:
			DecartProizv(pwB, pwA, pwL, B, A, L, n);                        //вызов функции декарового произведения
			break;															//окончание операции и возврат в меню
...
void Merge(int pwA, int pwB, int pwD, int A[], int B[], int D[])	//объединение множеств А и В (1 операция)
{
	...
	for (int i = 1; i <= pwB; i++)									//перебор всех элементов множества В по порядку (начало цикла 1)
	{
		...
		{
			pwD++;													//увеличение мощности множества D на единицу
...
{
	cout << endl << endl;
	cout << "Симметрическая разность множеств A и B" << endl;		
...
void Addition(int pwA, int pwH, int pwU, int A[], int H[], int U[])
{
	cout << endl << endl;
	cout << "Универсальное множества U" << endl;					
	for (int x = 1; x <= pwU; x++)
	...
	{
		for (int j = 1; j <= pwA; j++)								//перебор всех элементов множества A по порядку (начало цикла 2)
		{
			if (U[i] == A[j])										//проверка элементов на строгое равенство
			{
				i++;												//взятие следующего элемента множества U
				j = 0;												//обнуление счетчика множества A
			}
		}															//возврат к началу цикла 2
		if (i <= pwU)												//выполнение до тех пор, пока универсум U не закончится
		{
			pwH++;													//увеличение мощности множества H на единицу
			H[pwH] = U[i];											//запись элемента из универсального множества U во множество K
		}
	}
	cout << "Дополнение множества " << endl;						//наименование операции
	for (int i = 1; i <= pwH; i++)
		cout << H[i] << " ";										//вывод множества H
	cout << endl;
}


void DecartProizv(int pwA, int pwB, int pwL, int A[], int B[], int* L[], int n[]) //декартово произведение множеств А и В (8 операция)
{
	cout << endl << endl;
	cout << "Декартово произведение множеств A и B" << endl;		
	for (int i = 1; i <= pwA; i++)									//перебор всех элементов множества А по порядку (начало цикла 1)
	{
		for (int j = 1; j <= pwB; j++)								//перебор всех элементов множества В по порядку (начало цикла 2)
		{
